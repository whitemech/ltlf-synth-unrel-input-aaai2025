...
void ltlf2fol_with_projection(ltl_formula* unproj, ltl_formula* proj,const std::set<string>& projectables) {
  int c = 1;
  string res; 
  // We only have one alphabet for the formula (this is needed, as when a variable only occurs in the backup formula,
  // it is not in the alphabet of the main formula)
  // Thus, we get both alpahebts
  ...
  // Print the formulas
  if(!P1.empty()) {
    // This declares the logic used 
    cout<<"m2l-str;"<<endl;
    // We only have (open) second-order variables
    cout<<"var2 ";
    set<string>::iterator it = P1.begin ();
    // We use upper-case variable names for everything and therefore convert the variable names to upper-case
    cout<<up(*it);
    it++;
    while (it != P1.end ()){
      cout<<", "<<up(*it);
  ...
  // For debug reasons, print sizes as comments
  std::cout << "# " << projectables.size() << " projectables" << std::endl;
  // We print the first formula
  std::cout << "(( ";
  res = trans_fol(unproj, 0, c);
  std::cout << res;
  // Universally abstract over all the propositions (hence, use all2 --> second-order quantification)
  std::cout << ") & (";
    for(const auto& var : projectables) {
      std::cout << "(all2 " << up(var) << " : (";
    }
    c = 1;
    // Print the backup formula
